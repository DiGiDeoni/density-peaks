...
char IN [50] = "birch1"; 		
char DG [20] = "rhodel";								
static int  	fctr	= 10;			//for file reading
static float  	X		= 0.1;			//for decision graph		
static int  	want	= 0;			//gaussian density or not		
static int  	brho	= 0;			//find halo or not	
static int 		KKK 	= 100;			//initial centroid count
...
long Peak(
	...
	if (lnth%2!=0)
		inf = (dens[inf] < dens[lnth-1]) ? lnth-1:inf;
